	...
	<body onload="document.verifyform.attemptnames.focus()">
		...
		<div id="wrap">
			<div class="verifyform">
				<br/>
				Before you get the location of this document, we first have to verify that
				the document with the number<?php
				echo "&nbsp;&nbsp;<b>" . $_GET['idnumber'] . "</b>&nbsp;&nbsp;";
				?>it's actually your lost document.
				</p>
					Type in your names as they appear on your Document into the text box
				...
				<form name="verifyform" method="post" action="verifyowner.php" id="verifyform">
					<input type="text" name="attemptnames" id="attemptnames"/>
					<p/>
					<input type="hidden" value="<?php echo $docnumber;?>" name="docnumber"/>
					<input type="hidden" value="<?php echo $itemtype;?>" name="itemtype"/>
					<input type="button" id="verifybutton" value="Verify document" class="button btn-large"/>
				</form>
			</div>
			<p/>
		</div>
		...
		include "footer.php";
		?>
		<script charset="UTF-8" type="text/javascript" src="jquery_ptk/js/jquery.js"></script>
